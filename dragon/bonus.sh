...
function dlConfig(){
    ...
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/xtoys/Scripts/main/dragon/deps/config.sample.sh > $sample_shell_path
...
# 下载 extra.sh
function dlExtra(){
    local extra_shell_path=$dir_script/extra.sh

    if [ ! -a "$extra_shell_path" ]; then
        touch $extra_shell_path
    fi
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/xtoys/Scripts/main/dragon/deps/extra.sh > $extra_shell_path
...
# 下载 fixDeps.sh
function dlFixDeps(){
    local fix_shell_path=$dir_shell/fixDeps.sh

    if [ ! -a "$fix_shell_path" ]; then
        touch $fix_shell_path
    fi
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/xtoys/Scripts/main/dragon/deps/fixDeps.sh > $fix_shell_path
...
function dlWskey(){
    ...
    curl -s --connect-timeout 3 https://raw.githubusercontent.com/Zy143L/wskey/main/wskey.py > $wskey_path
